...
dotenv.config(); // 주로 맨 위에 작성(그래야 밑에서 쓸 수 있음)
...
// GET요청 / 주소(경로) 응답코드 응답에걸린시간 응답길이(바이트단위)
...
// body-parser: 옛날에는 body-parser를 많이 썻지만 요새는 express에 내장되서 안씀
...
const uploads = multer({
  storage: multer.diskStorage({ // 하드디스크에 저장(실제 서버 운영 시 서버 디스크 대신에 클라우드 스토리지 서비스에 저장하는게 좋음)
    ...
    // done(에러시 에러값, 성공 시 전달할 값); // 에러 발생 시 에러 처리 미들웨어로 
  }),
...
// app.post('/upload', uploads.single('image'), (req, res) => { // 라우터 전에 미들웨어로 장착
//   console.log(req.file); // 업로드 성공 시 정보가 저장됨
//   console.log(req.body); // { title: '' }
//   res.send('ok');
// });
// 2) 여러 파일을 업로드 하는 경우 array 미들웨어 사용
app.post('/upload', uploads.array('image'), (req, res) => { // 라우터 전에 미들웨어로 장착
  console.log(req.files); // 이 때는 file이 아닌 files임
  console.log(req.body); // { title: '' }
  res.send('ok');
});
// 3) 여러 파일(input 태그를 여러 개 사용해서 name 다른경우)을 업로드 하는 경우
// app.post('/upload', 
//   uploads.fields([{name: 'image'}, {name: 'image1'}]),
//   (req, res) => { 
...
//   }
// );
// 4) 멀티파트로 보내는데 파일을 업로드 하지 않을 때(잘 안씀)
// app.post('/upload', uploads.none(), (req, res) => { 
//   console.log(req.files); // undfined
...
app.get('/', (req, res) => {
  ...
  req.session.name = 'Jin'; // 세션 등록하면 세션 쿠키는 알아서 내려줌
  // 세션 ID 확인
  // 해당 세션 ID로 세선에 등록된 정보가 없으면 세션 ID는 요청마다 새롭게 생성됨
