...
// app.set(key, value): 서버에 전역 속성 설정
...
app.use((req, res, next) => {
  ...
  next(); // 다음 실행(미들웨어 또는 라우터) 흐름으로 넘어감
});
...
app.get('/', (req, res) => { // GET 요청이고 URL이 '/'(루트) 일 때
  ...
  // html 파일 서빙하기
  res.sendFile(path.join(__dirname, '/index.html')); // 알아서 fs모듈로 html 파일을 읽어서 부름
  ...
  // res.json({ name: 'Jin', age: 28 }); // res.writeHead(200, { .. }) + res.end(JSON.stringify({ .. })); 를 합친것!
...
app.use((err, req, res, next) => {
  ...
  res.status(500).send('에러가 나서 보여주는 내용'); // 응답 코드를 따로 작성하지 않으면 기본값 200
...
// 서버 실행법 3가지
// 1) node app
// 2) nodemon app(글로벌 설치 또는 npx 사용시)
