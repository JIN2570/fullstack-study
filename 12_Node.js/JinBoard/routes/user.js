...
// Qize: 회원 가입 기능 만들기
// 1) 회원 가입 페이지 만들기
// 2) 서버는 전송받은 아이디, 비번을 회원(user collection) DB에 저장
...
router.post('/login', isNotLoggedIn, checkIdAndPw, async (req, res, next) => {
  // 전송 받은 아이디, 비번이 DB에 있는지 확인하고 있으면 세션 만들기
  // 이 과정을 직접 만들기보다 passport의 미들웨어를 이용하여 로컬 로그인 전략을 수행
  passport.authenticate('local', (authError, user, info) => { // 전략이 성공하거나 실패하면 실핼될 콜백 함수
    // user: 성공 시 로그인 한 사용자 정보
    // info: 실패 시 이유
    if (authError) {
