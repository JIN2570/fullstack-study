  ...
  try {
    ...
    if (!title) {
      ...
      await db.collection('post').insertOne({ 
        ...
        // 2) 몽구스의 populate, 몽고DB의 aggregate 연산자 중 $lookup
      });
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    const result = await db.collection('post').updateOne({
      ...
      user: new ObjectId(req.user._id) // 본인이 쓴글만 수정되도록 조건추가
    }, {
      $set: { title, content }
    });
...
// post 컬렉션 document 안에 comments 필드를 만들어서 배열로 저장
// 근데 이 방식은 댓글이 많아지면 문제가 복잡해지고 비효일적임
