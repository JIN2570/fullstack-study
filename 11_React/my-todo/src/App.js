...
// 아이콘 리스트와 사용법은 공식 문서 참고:https://github.com/react-icons/
...
function App() {
  ...
  // 추가, 수정, 삭제 각 함수에 넣어도 되지만, useEffet()를 활용하면 한번에 처리 가능!
  // todos가 변경될 때마다 실행해라!
  useEffect(() => {
  ...
  // 새 객체를 만들때마다 id값에 1씩 더해줘야 하는데 useRef()를 사용하여 변수 생성
  // id값은 렌더링되는 정보가 아님(화면에 보이지도 않고, 이값이 바뀐다고 해서 컴포넌트가 재렌더링 될 필요도 없음)
