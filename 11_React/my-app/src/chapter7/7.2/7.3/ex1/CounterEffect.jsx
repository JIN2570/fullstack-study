...
function CounterEffect(props) {
  ...
  }); // componentDidMount, componenDidUpdate와 같은 방식으로 동작
  // }, []); // componentDidMount와 같은 방식으로 동작
  ...
  return (
    ...
    // document.title = `You clicked ${count} times`;
        // 차이점
        // 1. 초기에 타이틀이 안바뀌어 있음(왜? 처음 마운트 시 에 실행안됨)
        // 2. 버튼 클릭 시 count 값 반영이 제대로 안됨(왜? set함수는 비동기 함수)

        // onClick과 useEffect는 실행 시점이 다르기에 동작이 다를수 밖에 없음(완전히 다른 용도)
        // onClick 속성으로 넣어준 콜백 함수는 '클릭 시 호출' 함수이고,
