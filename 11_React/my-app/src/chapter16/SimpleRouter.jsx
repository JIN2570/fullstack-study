  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        {/* 방법1. 서브 경로 설정 */}
        {/* <Route path='game/hot' element={<HotGamePage />} />
        ...
        <Route path="/games" element={<GamePage />}>
          <Route path="hot" element={<HotGamePage />}></Route>
          <Route path="new" element={<NewGamePage />}></Route>
        </Route>

        {/* URL 파라미터와 쿼리스트링(Querystring) */}
        {/* URL 파라미터는 주소의 경로에 유동적인 값을 넣는 형태고,
          쿼리스트링은 주소의 뒷부분에 ? 문자열 이후에 key=value 형태로 값을 정의하며 &로 구분을 하는 형태
        */}
