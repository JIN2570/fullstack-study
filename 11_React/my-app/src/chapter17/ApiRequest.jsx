...
function ApiRequest(props) {
  ...
  const handleRequestById = (id) => {
    // jsonplasceholder에서 제공하는 테스트용 API를 호출 
    axios.get(`http://jsonplaceholder.typicode.com/photos/${id}`)
    .then((response) => {
      console.log(response);
      setData(response.data); // axios 라이브러릭 JSON => 객체로 변환해 주는 직업을 자동으로 해줌
    })
...
// Asynchronous JavaScipt And XML, 비동기 자바스크립트와 XML

// Ajax 쓰는 다양한 방법
// 1. XMLHttpRequest(XHR) 객체 - (ES6차 이전 방식이라 잘안씀)
